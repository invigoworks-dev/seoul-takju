...
router.post('/', async (req, res, next) => {
  try {
    ...
    try {
      ...
      await logInsert(client, 'raw_material_ledger', rows[0].id, rows[0], req.user?.id);
      if (material_id) {
        await propagateCarryOver(client, 'raw_material_ledger', 'material_id', material_id, ledger_date);
      }
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    try {
      await client.query('BEGIN');
      const existing = await client.query('SELECT * FROM raw_material_ledger WHERE id=$1 FOR UPDATE', [req.params.id]);
      if (!existing.rows.length) { client.release(); return res.status(404).json({ error: 'Not found' }); }
      const cur = existing.rows[0];
      const material_id     = req.body.material_id     ?? cur.material_id;
      ...
      await logUpdate(client, 'raw_material_ledger', cur.id, cur, rows[0], req.user?.id);
      if (material_id) {
        await propagateCarryOver(client, 'raw_material_ledger', 'material_id', material_id, cur.ledger_date);
      }
      await client.query('COMMIT');
      res.json(rows[0]);
    } catch (err) {
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    await pool.query('DELETE FROM raw_material_ledger WHERE id=$1', [req.params.id]);
    await logDelete(pool, 'raw_material_ledger', old.id, old, req.user?.id);
    res.status(204).end();
  } catch (err) {
    next(err);
  }
