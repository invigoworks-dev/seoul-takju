...
router.post('/', async (req, res, next) => {
  try {
    ...
    try {
      ...
      await logInsert(client, 'starter_ledger', rows[0].id, rows[0], req.user?.id);
      if (batch_code) {
        await propagateCarryOver(client, 'starter_ledger', 'batch_code', batch_code, ledger_date);
      }
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    try {
      await client.query('BEGIN');
      const existing = await client.query('SELECT * FROM starter_ledger WHERE id=$1 FOR UPDATE', [req.params.id]);
      ...
      await logUpdate(client, 'starter_ledger', cur.id, cur, rows[0], req.user?.id);
      if (batch_code) {
        await propagateCarryOver(client, 'starter_ledger', 'batch_code', batch_code, cur.ledger_date);
      }
      await client.query('COMMIT');
      res.json(rows[0]);
    } catch (err) {
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    await pool.query('DELETE FROM starter_ledger WHERE id=$1', [req.params.id]);
    await logDelete(pool, 'starter_ledger', old.id, old, req.user?.id);
    res.status(204).end();
  } catch (err) { next(err); }
});
